...
export class ResumesService {
  ...
  private async parseResumeAsync(resumeId: string, filePath: string): Promise<void> {
    try {
      ...
      if (resume && result.skills && result.skills.length > 0) {
        await this.skillsService.createPersonalizedTest(resume.userId, result.skills);
      }

      await this.resumeRepo.update(resumeId, { parsingStatus: 'completed' });
    } catch (e) {
      console.error('Resume parsing failed:', e);
      await this.resumeRepo.update(resumeId, { parsingStatus: 'failed' });
      throw e;
    }
  }
